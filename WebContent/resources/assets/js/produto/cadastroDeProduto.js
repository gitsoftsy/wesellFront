...
$(document).ready(function () {
  ...
  fetchDataActive("/categorias", "#categoria", "idCategoria", "categoria").then(
    () => {
      if (idProduto) {
        listarImagens();
...
function addImageCard(img) {
  var imageUrl = img.imagem.replace("/opt/tomcat9/webapps", url_image);
...
function validarComissao(comissao, precoPromocional, precoDeVenda, card) {
  const messageId = `errMessage${card.attr("id")}`;
  const comissaoFormatada = formatarValor(comissao);
  const precoPromocionalFormatado = formatarValor(precoPromocional);
  const precoDeVendaFormatado = formatarValor(precoDeVenda);
  let mensagem = "";

  if (comissaoFormatada === 0) {
    mensagem = "Comissão não pode ser zero.";
  } else if (
    precoPromocionalFormatado === 0 &&
    comissaoFormatada > precoDeVendaFormatado
  ) {
    mensagem = "Comissão não pode ser maior que o preço de venda.";
  } else if (
    precoPromocionalFormatado > 0 &&
    comissaoFormatada > precoPromocionalFormatado
  ) {
    mensagem = "Comissão maior que preço promocional.";
  }

  return exibirMensagemErro(card, messageId, mensagem);
}

function validarPrecoPromocional(precoPromocional, precoDeVenda, card) {
  const messageId = `errMessage${card.attr("id")}`;
  const precoPromocionalFormatado = formatarValor(precoPromocional);
  const precoDeVendaFormatado = formatarValor(precoDeVenda);
  let mensagem = "";

  if (precoPromocionalFormatado > precoDeVendaFormatado) {
    mensagem = "Preço promocional maior que preço de venda.";
  }

  return exibirMensagemErro(card, messageId, mensagem);
}

function validarPrecoDeVenda(precoDeVenda, card) {
  const messageId = `errMessage${card.attr("id")}`;
  const precoDeVendaFormatado = formatarValor(precoDeVenda);
  let mensagem = "";

  if (precoDeVendaFormatado === 0) {
    mensagem = "Preço de venda não pode ser zero.";
  }

  return exibirMensagemErro(card, messageId, mensagem);
}

function exibirMensagemErro(card, messageId, mensagem) {
  $(`#${messageId}`).remove();
  if (mensagem) {
    const message = $(`<p id='${messageId}'></p>`)
      .text(mensagem)
      .css("color", "#FF0000");
    card.append(message);
    return false;
...
function validarFormulario() {
  ...
  const validacaoComissao = validarComissao(
    comissao,
    precoPromocional,
    precoDeVenda,
    $("#cardComissao")
  );
  const validacaoPrecoPromocional = validarPrecoPromocional(
    precoPromocional,
    precoDeVenda,
    $("#cardPrecoPromocional")
  );
  const validacaoPrecoDeVenda = validarPrecoDeVenda(
    precoDeVenda,
    $("#cardPrecoDeVenda")
  );

  if (validacaoComissao && validacaoPrecoPromocional && validacaoPrecoDeVenda) {
    $("#btn-submit").removeAttr("disabled");
    return true;
  } else {
    $("#btn-submit").attr("disabled", "disabled");
