...
function validarValores(texto, valorMaior, valorMenor, container, card) {
	const box = container
	const message = $(`<p id='errMessage${card.attr('id')}'></p>`).text(texto).css('color', '#FF0000')
	...
	if (Number(valorMaior.replace(",", ".")) > Number(valorMenor.replace(",", "."))) {
		...
		$(`#errMessage${card.attr('id')}`).css('display', 'none')
	} else {
...
$("#comissao").blur(function() {
	if ($("#comissao").val() !== '' && $("#precoDeVenda").val() !== '') {
		validarValores(
	...
	if ($("#comissao").val() !== '' && $("#precoPromocional").val() !== '') {
		validarValores(
...
$("#precoDeVenda").blur(function() {
	if ($("#precoDeVenda").val() !== '' && $("#precoPromocional").val() !== '') {
		validarValores(
	...
	if ($("#comissao").val() !== '' && $("#precoDeVenda").val() !== '' && $("#comissao").val() < $("#precoDeVenda").val()) {
		validarValores(
			'Comissão maior que preço de venda.',
			$("#precoDeVenda").val(),
			$("#comissao").val(),
